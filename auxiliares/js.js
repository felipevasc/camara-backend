...
var listaOrdenada = function (event, ui) {

}

function selecionarVereador(vereador) {
    confirma('Deseja colocar o verador selecionado para o uso da palavra?', function () {
        chamarPaginaAjax('selecionar_vereador.php?vereador=' + vereador);
    });

}

function atualizaTempos() {
    if (parseInt(horas, 10) < parseInt($('#valor_horas').val(), 10)) {
        $('.reloj').css('color', '#363431');
    } else if (parseInt(horas, 10) == parseInt($('#valor_horas').val(), 10)) {
        if (parseInt(minutos, 10) < parseInt($('#valor_minutos').val(), 10)) {
            $('.reloj').css('color', '#363431');
        } else if (parseInt(minutos, 10) == parseInt($('#valor_minutos').val(), 10)) {
            if (parseInt(segundos, 10) <= parseInt($('#valor_segundos').val(), 10)) {
                $('.reloj').css('color', '#363431');
            } else {
                $('.reloj').css('color', '#FF3431');
            }
        } else {
            $('.reloj').css('color', '#FF3431');
        }
    ...
    $('#tempo_total').val(('00' + $('#valor_horas').val()).slice(-2) + ':' + ('00' + $('#valor_minutos').val()).slice(-2) + ':' + ('00' + $('#valor_segundos').val()).slice(-2));
}
function adicionarTempo() {
    $('#adicionar_tempo').attr('disabled', 'disabled');
    chamarPaginaAjax('adicionar_tempo.php', '', function () {
        $('#adicionar_tempo').removeAttr('disabled');
    });
}
function alteraTempoPrevisto(tempo) {
    var horas = Math.floor(tempo / (60 * 60));
    tempo = tempo - (horas * 60 * 60);
    var minutos = Math.floor(tempo / 60);
    tempo = tempo - (minutos * 60);
    var segundos = tempo;
    $('#valor_horas').val(horas);
    $('#valor_minutos').val(minutos);
    $('#valor_segundos').val(segundos);
}
var inicioSessao = '';
function relogioSessao() {
    if (inicioSessao == '') {
        $('#relogio_sessao').html("00:00:00");
    }
    var dataInicio = inicioSessao;
    var dataFim = new Date();
    var diffMilissegundos = (dataFim - dataInicio) + (3 * 60 * 60 * 1000);
    var diffSegundos = diffMilissegundos / 1000;
    var diffMinutos = diffSegundos / 60;
    var diffHoras = diffMinutos / 60;
    var horas = ("00"+diffHoras).slice(-2);
    var diffDias = diffHoras / 24;
    var diffMeses = diffDias / 30;
    $('#relogio_sessao').html((new Date(diffMilissegundos)).toTimeString().substr(0, 8));
    setTimeout(function() {
        relogioSessao();
    }, 1000);
}